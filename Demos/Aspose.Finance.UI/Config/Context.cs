...
namespace Aspose.Finance.UI.Config
{
    ...
    public abstract class Context
    {
        ...
        /// </summary>
        protected Dictionary<string, string> BaseResources
        {
            get
            {
                var sessionId = Configuration.ResourceFileSessionName + _context.Request.Url.Host.Trim().Replace(".", "");
                sessionId = "R" + sessionId;
                if (Cache[sessionId] == null)
                {
                    Session["AsposeAppResources"] = new GlobalAppHelper(
                        HttpContext.Current,
                        Configuration.ResourceFileSessionName + _context.Request.Url.Host.Trim().Replace(".", ""),
                        Locale
                    );
                }

                return (Dictionary<string, string>) Cache[sessionId];
            }
        }
