...
namespace Aspose.Finance.UI.Models
{
    public class ResourcesModel
    {
        ...
        public ResourcesModel(
            ...
            string product,
            string method
        ) : this(controller, extension: extension, product: product)
        {
            this.Method = method;
        ...
        private void GenerateKeys(string method)
        {
            ...
            if (Resources?.ContainsKey($"{Product}{method}H1") == true)
                ProductTitleKey = $"{Product}{method}H1";
            else
                ProductTitleKey = $"{Product}{method}Title";

            if (Resources?.ContainsKey($"{Product}{method}H4") == true)
                ProductTitleSubKey = $"{Product}{method}H4";
            else
                ProductTitleSubKey = $"{Product}{method}TitleSub";

            if (Resources?.ContainsKey($"{Product}{method}PageTitle") == true)
                PageTitleKey = $"{Product}{method}PageTitle";
            else
                PageTitleKey = ProductTitleKey;

            if (Resources?.ContainsKey($"{Product}{method}MetaDescription") == true)
                MetaDescriptionKey = $"{Product}{method}MetaDescription";
            else
                MetaDescriptionKey = ProductTitleSubKey;
        ...
        public string ProductsAsposeToolsURL => Config.Configuration.ProductsAsposeToolsURL;
        ...
        {
            UploadFile = new UploadFileModel(this.Resources)
            {
        ...
        public bool KeyExists(string key) => FindKey(Method, key, false) != null;
